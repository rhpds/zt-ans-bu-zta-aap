...
OAUTH_TOKEN=$(curl -sk -X POST -u "admin:${AAP_PASS}" \
  ...
  -d '{"description":"runtime-module05","application":null,"scope":"write"}' \
...
echo "Token obtained. Removing Section 4 templates..."
ansible-playbook -i "${PLAYBOOK_DIR}/inventory/hosts.ini" \
  "${PLAYBOOK_DIR}/setup/configure-aap-project.yml" \
  --tags remove_section \
  --extra-vars "aap_remove_section=4 controller_oauthtoken=${OAUTH_TOKEN}"

echo "Creating Section 5 templates..."
ansible-playbook -i "${PLAYBOOK_DIR}/inventory/hosts.ini" \
  "${PLAYBOOK_DIR}/setup/configure-aap-project.yml" \
  --tags section5,rbac \
  --extra-vars "controller_oauthtoken=${OAUTH_TOKEN}"

echo "Module 05 runtime setup complete."
