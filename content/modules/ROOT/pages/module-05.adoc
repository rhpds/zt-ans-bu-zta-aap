...
* EDA listening (Controller or `ansible-rulebook`).
...
====
Splunk answers “**do we see attack-like behaviour?**” The webhook is a **standardised signal** into **Event-Driven Ansible**, which turns that boolean into **approved automation** (run job template). This is the ZTA pattern of **policy-driven orchestration**: response playbooks are still **subject to AAP controls**, not anonymous shell scripts.
====

== Exercise 5.3 — EDA rulebook

* Controller: import `section5/eda/splunk-credential-revoke.yml`, create activation, decision environment, ensure listener port matches Splunk webhook.
* CLI example:
...
* AAP: **Emergency: Revoke App Credentials** ran without manual launch.
...
* [ ] Pre-attack healthy
* [ ] Splunk alert + webhook + EDA + AAP revoke
