...
Navigate to 'Apps' -> 'Search and Reporting'. From here we can verify that Splunk is receiving data from our infrastructure. Run the following search:
...
This should return with a number of indexes for the infrastructure.
...
Once created, there is a URL that is generated. This URL is the single endpoint for your webhooks. You can have multiple webhooks deliver to this endpoint and all payloads will be routed internally. Copy this generated URL as we will need it to update Splunk.
...
Now we can trigger a simulated brute force. Run the template 'Simulate Brute Force'. Launch this template and then navigate to Splunk.
...
Navigate to 'Settings' -> 'Searches, reports and alerts'. Change the fields to:
...
Now, let's go back to AAP. Click on Event Streams, and you will notice you have had a number of events come in, so we know the integration is working between Splunk and EDA. Click on the event stream and scroll down to see the event payload that has been received:
...
Time to simulate an attack. Login as the appdev user and see how EDA can assist without manual intervention. First, navigate to 'Templates' and edit the 'Deploy Application Pipeline' workflow, in Extra variables add the following:
