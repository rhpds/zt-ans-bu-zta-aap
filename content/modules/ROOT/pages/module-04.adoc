...
== Exercise 4.1 — Deny at the outer and inner rings
...
**Path A—Outer ring blocks (expected with correct LDAP maps):** `appdev` has no correct AAP team membership, so the `aap.gateway` policy denies the launch at the platform level. The playbook never starts and no SPIFFE check occurs. This is the same pattern as Section 3.
...
== Exercise 4.2 — Authorised VLAN creation with boundaries
...
== Exercise 4.3 — Verify SPIFFE trust and rogue workload denial
...
=== Legitimate vs rogue workload identity
...
== Corrected NetBox Update details:
[cols="1,3",options="header"]
|===
| Task | Key detail
