...
**Why that matters:** An attacker now needs to compromise *both* a valid user session *and* a trusted execution environment to change the network fabric. Neither identity alone is sufficient — this is **defence in depth** with independent verification at each layer, not a single trust decision repeated twice.
====
...
[%collapsible]
====
. Time to log into AAP as `appdev` and launch the **Configure VLAN** template with VLAN 200 / DMZ.
. Look at the output of the template:
...
. Now login as netadmin and re-run the Configure VLAN template, create VLAN 200.
. Navigate to the Netbox tab, and login with the credetnials admin/netbox.
. Click on IPAM, Select VLANS and you should see VLAN 200 now that has been created.
